...
void welcome()
{
	...
	cout << "  1) Add New employee" << endl; cout << "  2) Update_serial" << endl; cout << "  3) Delete_serial" << endl;
	cout << "  4) Display All Employees" << endl; cout << "  5) Display All Employees_Department" << endl; cout << "  6) Search_serial" << endl;
	cout << "  7) Compress file" << endl; cout << "  8) Exit" << endl << endl;
...
void add(fstream& file)
{
	...
	cout << "  ENTER THE SERIAL:";         cin >> s.serial;
	for (int m = 0; m < i; m++)
	{
		int n = 0;
		for (n = 0; n < 5; n++)
		...
		if (n == 5)
		{
			cout << "This serial is Already exists!" << endl
				<< "please enter anthor serial :" << endl;
			cin >> s.serial;
			m = -1;
		}
	}
...
void delet(fstream& file, string search)
{
	...
	if (pos == -1)
		cout << "  this serial not find in this file !" << endl;
	else
	{
		file.seekg(0, ios::beg);
		file.read((char*)&head, sizeof(short));
...
void Display_All(fstream& file)
{
	...
	{
		cout << endl;
		cout << "          Serial |  Firstname  |  Lastname  | Age | Department |  Salary  " << endl;
...
void display_department(fstream& file, string search)
{
	...
	if (b == false)
		cout << "  Not found this department!" << endl;
}
void search_by_serial(fstream& file, string search)
{
	...
	if (pos >= 0)
	{
		...
		cout << "  Not found this serial!" << endl;
}
void compress_file(fstream& file)
{

	fstream temp, clean; employee e;
	short head = -1, length = 0; int c = 0;
	string search;
	temp.open("temp_file.txt", ios::in | ios::out | ios::binary | ios::trunc);
	if (temp.is_open())
	{
	...
	for (int a = 0; a < j; a++)
	{
		...
		if (f == false)
		{
			...
			{
				for (int k = 0; k < 5; k++)
					sec[g].serial[k] = sec[g + 1].serial[k];
				for (int k = 0; k < 10; k++)
					sec[g].department[k] = sec[g + 1].department[k];
			}
			j--; a--;
		}
	}
	file.close();
	clean.open("project.txt", ios::in | ios::out | ios::binary | ios::trunc);
	if (temp.is_open())
	{
		clean.seekp(0, ios::beg);
		clean.write((char*)&head, sizeof(short));
		temp.seekg(2, ios::beg);
		while (c < i)
		{
			temp.read((char*)&length, sizeof(short));
			temp.seekg(-2, ios::cur);
			read(temp, e);
			prm[c].byte = clean.tellg();
			write(clean, e, length);
			c++;
		}
	}
	temp.close();
	...
	if (file.is_open())
	{
		...
		while (1)
		{
			...
			else if (ch == '2')
			{
				...
				cout << "  Enter the serial which want to update:";
				cin >> search;
				update(file, search);
			}
			else if (ch == '3')
			{
				...
				cout << "  Enter the serial which want to delete:";
				cin >> search;
				delet(file, search);
			}
			else if (ch == '4')
			...
			else if (ch == '6')
			{
				...
				cout << "  Enter the serial which want to display:";
				cin >> search;
